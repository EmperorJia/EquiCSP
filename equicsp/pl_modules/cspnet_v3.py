...
from equicsp.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPLayerSelfAttn(nn.Module):
    ...
    def edge_model(self, node_features, frac_coords, lattices, edge_index, edge2graph, frac_diff = None):
        ...
        q, k, v = map(lambda t: rearrange(t, 'e (h d) -> e h d', h = self.head), (q, k, v))
        scores = (q * k).sum(dim = -1) / math.sqrt(self.head_dim)   
        attn = scatter_softmax(scores, edge_index[0], dim = 0).unsqueeze(-1)
        agg = scatter(attn * v, edge_index[0], dim = 0, reduce='sum', dim_size=node_features.shape[0])
